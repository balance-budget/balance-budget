        ...
        <>
            {justCreated ? (
                <Panel className="border-warning/40 bg-warning-soft">
                    ...
                    />
                    <code className="block px-3 py-2 rounded-sm bg-bg-1 border border-border-soft font-mono text-[13px] text-fg-1 break-all">
                        {justCreated.token}
                    </code>
                    <div className="mt-3">
                        <button
            ...
            <Panel>
                ...
                {tokensQuery.isPending ? (
                    ...
                    <p className="text-[13px] text-fg-3">No tokens yet.</p>
                )}
            </Panel>
                ...
                >
                    ...
                    <label className="flex flex-col gap-1 mb-3">
                        ...
                        <input
                            ...
                            className="px-3 py-2 rounded-sm bg-surface-2 border border-border-soft text-fg-1 text-[14px] placeholder:text-fg-4 focus:outline-none focus:border-border-strong"
                        />
                    </label>
    ...
    return (
        <li className="flex items-center justify-between gap-3 px-3 py-[10px] rounded-sm bg-surface-2 border border-border-soft">
            <div className="min-w-0 flex-1">
                <div className="text-[14px] font-medium text-fg-1 truncate">{token.name}</div>
            ...
            <div className="shrink-0">
                {isRevoked ? (
                    <span className="px-2 py-[3px] rounded-xs text-[12px] font-medium bg-danger-soft text-danger">
                        Revoked
                    </span>
                ) : isExpired ? (
                    <span className="px-2 py-[3px] rounded-xs text-[12px] font-medium bg-surface-3 text-fg-3">
                        Expired
                    </span>
                ) : (
                    <button
