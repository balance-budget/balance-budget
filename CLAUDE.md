...
- **Comments.** Only explain what is not directly obvious from the code. No prose, no narration, no historical or changelog information (don't describe what changed, what a value used to be, or which ADR superseded which). Don't restate what the code already says. A file's comments should read the same whether it was written today or refactored ten times.
