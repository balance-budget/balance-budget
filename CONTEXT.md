...
An **Asset** **Account** holding mortgage money the lender has *not yet disbursed* — funds earmarked for building or renovation that you draw down as invoices come in (Dutch: *bouwdepot*). A plain, **not Loan-managed** Account whose monthly statement is imported like any other **BankAccount**, keyed by a non-IBAN `AccountNumber` equal to the loan number (the statement carries no counterparty account numbers). Its statement has three row kinds: a **Deposit-interest credit** (*Rentevergoeding*), a **Deposit settlement** (*Verrekening*), and draw-downs (*Uitbetaling* — the lender disbursing to a contractor, or reimbursing you into Checking as a **Self-transfer**). A **Loan** *may* reference one Construction deposit, alongside a deposit-interest **Income** **Account** and a single editable annual rate, used to compute the **Deposit-interest credit** and matching **Deposit settlement** during construction. Typically **Illiquid** (earmarked, not day-to-day money).
...
The interest the lender pays *into* the **Construction deposit** each month (Dutch: *Rentevergoeding*), recorded as its own **JournalEntry**: the deposit **Asset** debited, the **Loan**'s configured deposit-interest **Income** **Account** credited. Computed as `Construction deposit balance at period start × monthly deposit rate`. Unlike the superseded netted offset (ADR-0037), this interest *lands in the deposit balance* and therefore compounds — it sits in the deposit for one period before the matching **Deposit settlement** withdraws it, so each period's interest is computed on a base that includes last period's still-unsettled credit. Created by **Categorizing** the *Rentevergoeding* **BankTransaction**; the loan-aware proposal pre-fills the income account and amount as editable defaults (the bank's figure wins).
_Avoid_: deposit-interest offset (the superseded netted model — ADR-0037), Rentevergoeding (Dutch).

**Deposit settlement**:
The withdrawal from the **Construction deposit** that funds part of a **Loan payment** during construction (Dutch: *Verrekening i.v.m. maandbedrag*), equal to the *previous* period's **Deposit-interest credit** — the lender settles one period in arrears. Modeled as a second funding leg on the **Loan payment** **JournalEntry** (a credit to the deposit **Asset**), so the entry's cash (bank) leg is the gross payment minus the settlement, matching the reduced debit the lender collects. Reconciled by **Attaching** the *Verrekening* **BankTransaction** to that leg. Carries no **Loan Part** attribution — it is a loan-level line. Drops to €0 once the deposit balance and its trailing credit are exhausted, reverting the **Loan payment** to gross.
_Avoid_: netting (describes the mechanism, not the line), deposit-interest offset (superseded netted model — ADR-0037).
...
**Inbox**:
The derived set of **BankTransactions** that have no referencing **JournalEntry** and no recorded **Dismissed** state — the starting point of the **Categorization flow**. *Derived*, not stored: the filter is `b.JournalEntryId IS NULL AND b.DismissedAt IS NULL`. Defaulted-sorted oldest-first so the user works the queue in statement order. Each Inbox row carries an optional **Attach hint** (`MatchingJournalEntryId`) when the **Attach predicate** uniquely identifies a **JournalEntry** the row should link to — a self-transfer, or a construction-phase **Loan payment** with an unsettled **Deposit settlement** leg (ADR-0037) — surfaces as a one-click `Attach` action alongside `Categorize` and `Dismiss`. Distinct from the full **BankTransaction** list view, which shows every imported row regardless of state.
...
- A **Loan** *may* reference one **Construction deposit** (an **Asset** **Account**), one postable **Income** **Account** for the **Deposit-interest credit**, and a single annual deposit rate — all three set together or not at all. The **Construction deposit** is *not* **Loan-managed**.
...
- A **BankTransaction** references at most one **JournalEntry** via the nullable scalar FK `BankTransaction.JournalEntryId`. A **JournalEntry** is referenced by zero, one, or many **BankTransactions**; multiple references are reserved for self-transfers (every line on an own-**Account**, `CounterpartyId IS NULL`) and for construction-phase **Loan payments** carrying a **Deposit settlement** leg (ADR-0037), enforced in the service layer as part of the **Attach predicate**. Splits are modeled as multiple **JournalLines** within one **JournalEntry**, not as multiple **JournalEntries** sharing a **BankTransaction**.
