...
**Typical spend**:
The modeled baseline of *non-recurring* variable flow on a **Liquid** balance-sheet **Account**, projected forward to make a balance **Projection** realistic — the everyday card spend, groceries, and fuel that no **JournalEntryTemplate** captures. Computed per liquid **Account**, per month, as the trailing-three-month **median** of that account's **Net movement** *excluding* (a) any **JournalEntry** matched to a **JournalEntryTemplate** by **Occurrence matching** (those are the recurring skeleton — counting them here double-counts) and (b) ad-hoc **Self-transfers** (unpredictable lump moves, not typical spend — recurring transfers are modeled as templates instead). *Derived, never stored*, like every other **Projection** input. Rendered as a **band** (a low/high range), never a single point, so the **Projection** advertises its own uncertainty rather than faking precision. The forward headline reads as *known commitments (templates) + Typical spend (baseline)*.
